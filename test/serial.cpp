...
int  ngb_Write(int fd,void* buffer,int size)
{
	...
	Thread::lock() ;
	if ( fd < 0 || NULL == buffer ){
		//LOGI("Write fd < 0 return -1") ;
		goto OUT ;
	}
	if( (write_size = write( fd,buffer,size )) == -1){
		//LOGI(" write  error info is [%s]\n",strerror(errno));
		goto OUT ;
	}
//	printf("Write Success Write_Size:%d!!!\n",write_size) ;
	Thread::unlock() ;
	return write_size;
OUT:
	Thread::unlock() ;
...
int  ngb_SetStopBits(int fd ,int digit)
{
	...
	struct termios term ;
	if ( tcgetattr( fd, &term ) < 0 ){
		//LOGI("ngb_SetStopBits tcgetattr( fd, &term ) < 0\n");
		return -1;
	}
	switch(digit){
	case 1:
	...
	if ( -1 == tcsetattr( fd, TCSANOW ,&term) ){
		//LOGI("ngb_SetStopBits tcsetattr( fd, &term ) < 0\n");
		return -1;
	}

	//LOGI("ngb_SetStopBits Success\n") ;
	return 0;
